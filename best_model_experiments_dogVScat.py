...
def do_stuff():
    ...
    model2.load_weights("weights.best2.hdf5")
    # Let's train the model using RMSprop
    ...
    plt.figure(2)
    plt.xlim(0,0.3)
    sns.kdeplot(uncertainty_correct[:len(uncertainty_incorrect)], label="Uncertainty of well clasificated images",color="g" ,shade=True);
    sns.kdeplot(uncertainty_incorrect, color="r",label="Uncertainty of incorrect clasificated images", shade=True);
    plt.legend()

    plt.figure(3)
    plt.hist(uncertainty_correct[:len(uncertainty_incorrect)], 30,label="Uncertainty of well clasificated images", density=True, facecolor='g', alpha=0.75)
    plt.grid(True)
    plt.hist(uncertainty_incorrect, 30,label="Uncertainty of incorrect clasificated images", density=True, facecolor='r', alpha=0.75)
    plt.grid(True)
    plt.legend()
    ...
    uncertainty_entropy = []
    for i in range(2000):
        uncertainty_entropy.append(-(y_predict3[i][1]*np.log(y_predict3[i][1])+(1-y_predict3[i][1])*np.log(1-y_predict3[i][1])))
    ...
    plt.figure(1)
    plt.plot(range(500, 1500), plot_retained[::-1],label="MC Dropout")
    plt.plot(range(500, 1500), plot_retained2[::-1],color="r",label="Normal dropout")
    plt.plot(range(500, 1500), plot_retained3[::-1], color="g",label="Entropy")
    plt.plot(range(500, 1500), plot_retained4[::-1], color="fuchsia", label="Random")
    plt.ylabel('AUC')
    plt.xlabel('Retained data')
    plt.savefig("retainedMCvsSMAXvsEntropyvsRandom.png")
    plt.legend()
    plt.show()
